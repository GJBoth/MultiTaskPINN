...
class Library(nn.Module):
    ...
    def forward(self, input: Tuple[TensorList, TensorList]) -> Tuple[TensorList, TensorList]:
        ...
        self.norms = [theta_norm for theta_norm, dt_norm in zip(theta_norms, time_deriv_norms)]
...
class DeepMoD(nn.Module):
    ...
    def __init__(self,
        ...
        self.loss_scale = self.initialize_scales()
    ...
    def initialize_scales(self) -> torch.nn.Parameter:
        """[summary]

        Returns:
            torch.nn.Parameter: [description]
        """
        # To determine nnumber of terms, we do a mock forward pass
        mock_X = torch.ones((1, self.func_approx.in_features), dtype=torch.float32, requires_grad=True)

        n_regression = len(self.library((self.func_approx(mock_X), mock_X))[0])
        n_fit = self.func_approx.out_features

        return torch.nn.Parameter(torch.zeros(n_fit + n_regression, dtype=torch.float32))